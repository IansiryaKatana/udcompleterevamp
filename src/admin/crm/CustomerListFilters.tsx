...
import type { CrmCustomerListFilters, CrmCustomerSort } from '@/admin/lib/crmOps'
...
type Props = {
  ...
  onChange: (next: CrmCustomerListFilters) => void
  onSortChange: (sort: CrmCustomerSort) => void
  onReset: () => void
}
...
export function CustomerListFilters({ filters, sort, facets, onChange, onSortChange, onReset }: Props) {
  const [open, setOpen] = useState(false)
  const set = (patch: Partial<CrmCustomerListFilters>) => onChange({ ...filters, ...patch })
  const activeCount = countActiveListFilters(filters as Record<string, unknown>)

  return (
    <>
      <AdminListFilterToolbar
        searchPlaceholder="Name, email, phone, StoreName, company, staff, tags, order #…"
        searchAriaLabel="Search customers"
        searchValue={filters.search ?? ''}
        onSearchChange={(v) => set({ search: v })}
        sort={sort}
        sortOptions={SORT_OPTIONS}
        onSortChange={(v) => onSortChange(v as CrmCustomerSort)}
      ...
      <AdminSheet
        ...
        title="Filters"
        description="Narrow the customer list. Changes apply as you go."
        footer="filters"
        onReset={onReset}
        size="lg"
      >
        <div className="space-y-6">
          ...
          <FilterSection title="Commercial">
            <ChipRow>
              <FilterChip
                active={filters.has_company === true}
                onClick={() => set({ has_company: filters.has_company === true ? null : true })}
              >
                Has company
              </FilterChip>
              <FilterChip
                active={filters.has_company === false}
                onClick={() => set({ has_company: filters.has_company === false ? null : false })}
              >
                No company
              </FilterChip>
              <FilterChip
