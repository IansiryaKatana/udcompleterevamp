...
import { formatCrmLabel, fmtCrmWhen } from '@/admin/lib/crmOps'
...
export function CrmActivityPanel({ items, total, canAddNote, onAddNote }: Props) {
            ...
            >
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wide text-[var(--admin-muted)]">
                  {isNote
                    ? 'Note'
                    : formatCrmLabel(String(ev.event_type || ev.category || 'event'))}
                </span>
                <span className="text-xs text-[var(--admin-muted)]">
                  {fmtCrmWhen(String(ev.occurred_at || ev.created_at || ''))}
                </span>
              </div>
