...
import { formatDraftStatusLabel, fmtDraftWhen } from '@/admin/lib/draftOps'
...
export function DraftActivityPanel({ items, total, canAddNote, onAddNote }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <ol className="space-y-3">
            ...
            >
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wide text-[var(--admin-muted)]">
                  {isNote ? 'Note' : formatDraftStatusLabel(String(ev.event_type || ev.category || 'event'))}
                </span>
                <span className="text-xs text-[var(--admin-muted)]">
                  {fmtDraftWhen(String(ev.occurred_at || ev.created_at || ''))}
                </span>
              </div>
              <p className="mt-1 whitespace-pre-wrap text-[var(--admin-text)]">
                {String(ev.body || ev.message || '—')}
              </p>
              {Boolean(ev.actor_name_snapshot || ev.source_system) && (
                <p className="mt-1 text-[11px] text-[var(--admin-muted)]">
                  {[String(ev.actor_name_snapshot || ''), String(ev.source_system || '')]
                    .filter(Boolean)
                    .join(' · ')}
                </p>
              )}
        ...
        {items.length === 0 && (
          <li className="py-6 text-center text-sm text-[var(--admin-muted)]">No activity yet.</li>
        )}
      </ol>
    </div>
  )
