-- Phase 5F selftest: valid orders.status values are lowercase pending/paid/...
...
begin
  ...
  select id, coalesce(nullif(email,''), 'phase5f-finance@unique.test') into v_cust, v_email
  from customers where coalesce(email,'') <> '' order by created_at desc nulls last limit 1;
  ...
  insert into orders (
    ...
    v_email, v_cust, 'pending', 'PENDING',
    100.00, 0, 100.00, 'GBP',
  ...
  update orders set total_received = 100, total_outstanding = 0, financial_status = 'PAID', status = 'paid'
  where id = v_order_id;
