-- Fix workspace RPC: orders has no trading_name column (use trading_name_snapshot if present).
  ...
  with filtered as (
    ...
    where (v_include_test or coalesce(o.is_test, false) = false)
      ...
      and (
        ...
        or coalesce(o.trading_name_snapshot, '') ilike '%' || v_search || '%'
        or exists (
          select 1 from public.fulfillments f
    ...
    coalesce((
      ...
      from (
        select
          ...
          o.order_number,
          o.email,
          o.trading_name_snapshot as trading_name,
